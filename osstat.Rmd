...
value <- round(mean(growth(data$nb_visits),na.rm=T)*100)
...
value <- round(mean(growth(data$nb_uniq_visitors),na.rm=T)*100)
...
value <- round(mean(growth(data$nb_actions),na.rm=T)*100)
...
value <- round(mean(growth(data$avg_time_on_site),na.rm=T)*100)
...
```{r}
data$bounce_rate <- as.numeric(as.character(data$bounce_rate))
value <- round(mean(growth(data$bounce_rate),na.rm=T)*100)
valueBox(paste0(value," %"), icon = "fa-futbol-o", color=ifelse(value<50,"success","warning"))
```
