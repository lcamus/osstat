...
c1 <- function() {
  hcmap(map = "custom/world", download_map_data=F, data=cc, joinBy=c("hc-key","countryCode"), value="nbVisits",
        name="number of visits") %>%
    hc_exporting(enabled=T) %>%
...
clean <- function(begin,end,agreg) {
  ...
  if (agreg)
    ...
    pa_sel <- pa_sel[,c("cat","nb_hits","date")]
...
p <- clean(begin="2017-03-01", end="2017-05-31",agreg=F)

require(viridis)
hchart(p, "heatmap", hcaes(x = date, y = cat, value = nb_hits)) %>% 
