...
setMarkersDatalabels <- function(data, var) {
  ...
  breaks <- list(
    list(which(max_cond),max(data[,c(var)])),
    list(which(min_cond),min(data[,c(var)])),
    list(nrow(data),tail(data[,c(var)],1))
  )
  hc_data <- lapply(seq(1,which(max_cond)-1),function(x){data[x,c(var)]})
  hc_data <- c(hc_data,setPoint(max(data[,c(var)])))
  hc_data <- c(hc_data,
               lapply(seq(which(max_cond)+1,length(max_cond)-1),function(x){data[x,c(var)]}))
  hc_data <- c(hc_data,setPoint(tail(data[,c(var)],1)))
...
c_dailyvisits <- function(df, lib) {
  ...
  hc_data <- setMarkersDatalabels(data,"nb_visits")
