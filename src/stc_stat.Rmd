...
freqdis <- function() {
  ...
  duration <- v[v$visitDuration<3600,]$visitDuration/60
  ...
  highchart() %>%
    ...
    hc_xAxis(
      categories = rownames(duration.cumfreq),
      title = list(text = "minutes")) %>%
    hc_series(
      list(
        name = "visits duration",
        data = setNames(duration.cumfreq[,1]*100,NULL)
      )
    ) %>%
    hc_tooltip(
      headerFormat="<b>{point.key}</b> minutes (visits duration)<br>",
